...
public class MainWindowTests
{
    ...
    public void _4_ShouldShowSumAndAverageAfter2Clicks()
    {
        ...
        double expectedSum = firstNumber + secondNumber;
        double actualSum = double.Parse(_sumTextBox.Text);
        double difference = Math.Abs(actualSum - expectedSum);
        Assert.That(difference <= 0.01, () => "After the second click the sum TextBox should contain the sum");

        // because you parse back the texboxes, there can be a difference
        // with the actual values, but not more than 0.01
        double actualAverage = double.Parse(_averageTextBox.Text);
        double expectedAverage = (firstNumber + secondNumber) / 2;
        difference = Math.Abs(actualAverage - expectedAverage);
        Assert.That(difference <= 0.01, () => "After the second click the average TextBox should contain the average");
    }
